using OrientEnergy.Controllers;
...
namespace OrientEnergy.Tests
{
    public class PricePlanComparisonTest
    {
        ...
        private static string SMART_METER_ID = "orient-sm-id";
        ...
        public PricePlanComparisonTest()
        {
            ...
            var pricePlans = new List<PricePlan>() { 
                new PricePlan() { EnergySupplier = Supplier.BravoEnergy, UnitRate = 10, PeakTimeMultiplier = NoMultipliers() }, 
                new PricePlan() { EnergySupplier = Supplier.TheGreenEco, UnitRate = 2, PeakTimeMultiplier = NoMultipliers() },
                new PricePlan() { EnergySupplier = Supplier.NeurondPower, UnitRate = 1, PeakTimeMultiplier = NoMultipliers() } 
            };
        ...
        public void ShouldCalculateCostForMeterReadingsForEveryPricePlan()
        {
            ...
            Assert.Equal(100m, actualCosts["" + Supplier.BravoEnergy], 3);
            Assert.Equal(20m, actualCosts["" + Supplier.TheGreenEco], 3);
            Assert.Equal(10m, actualCosts["" + Supplier.NeurondPower], 3);
        }
        ...
        public void ShouldRecommendCheapestPricePlansNoLimitForMeterUsage()
        {
            ...
            Assert.Equal("" + Supplier.NeurondPower, recommendations[0].Key);
            Assert.Equal("" + Supplier.TheGreenEco, recommendations[1].Key);
            Assert.Equal("" + Supplier.BravoEnergy, recommendations[2].Key);
